...
import cn.edu.sicnu.cs.manager.MyAccessDecisionManager;
...
public class MyFilterSecurityInterceptor extends AbstractSecurityInterceptor implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (logger.isDebugEnabled()){
            logger.debug("MyFilterSecurityInterceptor 拦截了 "+request.getRemoteHost()+"  "+request.getRemoteAddr());
        }

        //解决同一请求，两次经过过滤器  原因：过滤器被加载WebSecurityConfig和spring都加载了
        if (request.getAttribute(FILTER_APPLIED) != null) {
            if (logger.isDebugEnabled()){
                logger.debug("MyFilterSecurityInterceptor 多次经过后直接放行 ");
            }
